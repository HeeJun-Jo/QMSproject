...
import com.qms.receive.vo.PartVO;
...
public class RcvController {
	...
	@Autowired
	PartnerService ptservice;
	// 입고조회
	@RequestMapping("/receive/list")
	public String receiveSearch(PartVO vo, HttpServletRequest request) throws Exception{
	...
	@ResponseBody
	public PartnerVO getPartnerList(@ModelAttribute("PartnerVO")PartnerVO vo)throws Exception{
		
		int totalPage = 0;
		
		PartnerPageVO pagevo = new PartnerPageVO();
		totalPage = ptservice.selectTotalPartnerCount(vo);
		List<PartnerVO> list = ptservice.selectPartnerList(vo);
